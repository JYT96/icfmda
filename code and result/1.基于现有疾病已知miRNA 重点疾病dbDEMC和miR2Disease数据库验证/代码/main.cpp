  ...
  for(i=0;i<Nd;i++)
    for(j=0;j<Nr;j++)
      if(AI(i,j)==0){
        ...
        if(dbDEMC(i,j)==1&&miR2Disease(i,j)==1) outFile<<"&dbDEMC;miR2Disease"<<endl;
        else if(dbDEMC(i,j)==1) outFile<<"&dbDEMC"<<endl;
        else if(miR2Disease(i,j)==1) outFile<<"&miR2Disease"<<endl;
        else outFile<<endl;
      }
  outFile.close();
  string path("results/");
